...
- `datasets/`：存放数据集的文件夹。
- `从零开始的机器学习/`：存放从零实现机器学习算法的相关代码。
- `基于sklearn库实现工程化机器学习/`：存放使用sklearn库进行机器学习的代码。
- `从零开始的机器学习/数学基础`：存放通过编写代码实现机器学习相关的数学基础知识。
- `从零开始的机器学习/深度学习`：存放分别使用Pytorch框架以及Tensorflow进行深度学习的代码。
- `资料/`：存放与机器学习相关的参考资料和文档。
- `README.md`：项目的自述文件，包含项目简介和使用说明。
- `LICENSE`：项目的许可证文件，说明项目的使用权限和限制。
- `tensorflow对应cuda.txt`：存放TensorFlow对应的CUDA版本信息。

### PS：本项目中的Tensorflow版本为2.10.0，对应的CUDA版本为11.2，CUDNN版本为8.1。
